...
| Localization automation | 校验 `en.lproj` / `zh-Hans.lproj` 已打入 App，并分别读取设置标题、动作名和更新菜单文案 | `make verify-localizations` |
...
14. macOS 首选语言为英文或简体中文时，状态栏菜单、设置、登录项状态、快捷键录制和更新提示使用对应语言，最小窗口尺寸下无截断或重叠。
...
- 修改本地化 helper、strings 资源、可见文案或 App bundle 资源复制。
- 修改显示器排序、选屏或跨屏相对位置计算。
- 修改 hotkey 注册。
- 修改登录项注册、状态映射或设置开关。
- 修改更新检查、下载验证或替换助手。
