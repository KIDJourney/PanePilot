...
## DEC-0007: App 代码提交必须自动形成正式 Release

| 字段 | 内容 |
|---|---|
| 日期 | 2026-08-14 |
| 状态 | 已接受 |
| 背景 | 本地预览包和已推送源码不能替代用户可安装、可自更新的正式版本；此前代码修复已推送但没有发布 |
| 决策 | `post-commit` 对 main 上影响 App、测试、资源、构建或发布工具的提交自动递增 patch 并执行本地正式发布；`pre-push` 阻止没有远端 tag、DMG 和 sha256 Release 的代码提交进入远端 main；纯文档提交不发布 |
| 理由 | 把“必须发布”从 Agent 判断改成可执行生命周期门禁，同时继续保证构建、签名、公证和验证全部发生在本机 |
| 影响 | code commit 可能等待 Apple 公证；失败后 commit 保留但不能推送 main，修复环境后运行 `make release-next` 重试 |
