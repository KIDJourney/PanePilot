...
| 日期 | 场景 | 变更 | 验证 | 结果 |
|---|---|---|---|---|
| 2026-08-14 | 英文与简体中文 | 菜单、设置、动作、登录项状态、权限和更新提示改为标准 App bundle 本地化，默认跟随 macOS；增加资源和代表性文案门禁 | `swift test`（15 tests）、`make verify-localizations`、中英文默认/最小尺寸及登录项待批准设置截图、`make local-check`、commit `2a15510` 自动发布 `v0.1.7`、最终签名 App 用 `AppleLanguages` 双向读取文案 | 通过；八张真实 AppKit 截图无截断、重叠或控件漂移；英文和简体中文 strings 均进入正式 App；App/DMG 公证、stapling、Gatekeeper、下载 sha256 和启动验证通过，DMG sha256 为 `bb7681c83a99de467b610ad60f900e00de7c540d3fcecf4841665528d8479641` |
| 2026-08-14 | 代码变更强制正式发布 | 新增 post-commit 自动 patch Release 和 pre-push 远端 Release 门禁；修正“只有明确要求才发布”的项目规则 | `PANEPILOT_AUTO_RELEASE_DRY_RUN=1 make release-next`、模拟未发布 main push、已发布 v0.1.5 gate、commit `a360f4c` 触发正式 `v0.1.6` 本地签名/公证/上传/下载校验/启动验证 | 通过；未发布 commit 被拒绝，既有 Release 通过；post-commit 自动发布 v0.1.6，App 和 DMG 公证均 Accepted，下载 sha256 `5996265518d589fd5a739fa6aaf4c49fab1cbac59450e4ab9cee833743de4bfe`，Gatekeeper accepted，最终 App 以版本 0.1.6 成功启动 |
| 2026-08-14 | Chrome 最大化后右半屏闪动修复 | AX frame 写入从 `Position -> Size` 改为 `Size -> Position -> Size`；写入期间临时关闭并恢复应用的 `AXEnhancedUserInterface`；增加写入顺序单测和隔离 Chrome 回归 | `swift build`、`swift test`（15 tests）、`make verify-chrome-transition`、`make verify-window-move`、`make local-check`（含 `make validate-docs`、本地 package 和 `codesign --verify --deep --strict`） | 通过；隔离 Chrome 在最大化后直接稳定到 Right Half，命令返回后只观察到右半屏 frame；AppKit 夹具移动通过；本地生成 `PanePilot-v0.1.5-local-b424951d8515-macos-arm64.zip` 且签名检查通过 |
